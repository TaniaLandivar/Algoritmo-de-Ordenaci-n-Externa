 ...
 * @author Mateo
 */
public class MezclaDirectaKeyFecha {
    public MezclaDirectaKeyFecha(String ruta) throws IOException, ParseException {
        File fichero = new File(ruta);
        mezclaDirecta(fichero);
    }
    
    private  void mezclaDirecta(File f) throws IOException, ParseException {
        int longSec;
        int numReg = 0;
   ...
   private void mezclar(File f1, File f2, File f,int lonSec, int numReg) throws IOException, ParseException{
        SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy"); 
        Date fechaclave1 = null,fechaclave2=null;
        ...
        clave1=bf1.get(3);
        System.out.println("fecha1: "+clave1);
        fechaclave1 = formato.parse(clave1);
        boolean is_end2 = bf2.readRecord();
        clave2=bf2.get(3);
        System.out.println("fecha2: "+clave2);
        fechaclave2 = formato.parse(clave2);
        ...
        for (int s = 1; s <= numSec+1; s++)
        {
            ...
            {
                String clave;
                if (fechaclave1.compareTo(fechaclave2)<0)
                {
                    clave = clave1;
                    getClave1=bf1.get(0);
                    getClave2=bf1.get(1);
                    ...
                    try {
                        ...
                        if(auxend1!=false){
                            clave1= clave1aux;
                            Date clave1auxD=formato.parse(clave1aux);
                            fechaclave1= clave1auxD;
                        }
                    }
                ...
                else
                {
                    ...
                    try {
                        ...
                        if(auxend2 !=false){
                            clave2= clave2aux;
                            Date clave2auxD=formato.parse(clave2aux);
                            fechaclave2= clave2auxD;
                        }
                    }
                    catch(EOFException e){;}
                    j++;
